...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  cell.accessoryType = [self.appSelectionController.selectedAppIDs
...
- (void)viewDidLoad {
  ...
  _appListDataSource.sectionDescriptors =
      [NSPAppSelectionALApplicationTableDataSource standardSectionDescriptors];
...
- (void)doneSelecting {
  if (callback) {
    callback(self.selectingMultiple
                 ? self.selectedAppIDs
                 : (self.selectedAppIDs.count ? self.selectedAppIDs[0] : nil));
  }
  [self dismiss];
}
