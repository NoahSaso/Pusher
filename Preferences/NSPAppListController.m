...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  cell.accessoryType =
      [self.appListController.selectedAppIDs containsObject:displayIdentifier]
...
- (void)viewDidLoad {
  ...
  _appListDataSource.sectionDescriptors =
      [NSPAppListALApplicationTableDataSource standardSectionDescriptors];
...
- (void)updateTitle {
  self.navigationItem.title =
      XStr(@"%@ (%d total)", _label, (int)self.selectedAppIDs.count);
...
- (void)updateSearchResultsForSearchController:
  ...
  NSMutableArray *sectionDescriptors =
      [NSPAppListALApplicationTableDataSource
  ...
  if (XIsEmpty(filter)) {
    ...
    for (NSDictionary *sectionDescriptor in sectionDescriptors) {
      ...
      filteredSectionDescriptor[ALSectionDescriptorPredicateKey] = XStr(
          ...
          filteredSectionDescriptor[ALSectionDescriptorPredicateKey], filter);
...
- (void)updatePreferencesForAppID:(NSString *)appID selected:(BOOL)selected {
  ...
  CFPreferencesSetValue((__bridge CFStringRef)key, @(selected), PUSHER_APP_ID,
...
- (void)tableView:(UITableView *)table
  ...
  [self updatePreferencesForAppID:displayIdentifier
                         selected:![self.selectedAppIDs
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  if (!_prefs[@"AppListTutorialShown"] ||
      !((NSNumber *)_prefs[@"AppListTutorialShown"]).boolValue) {
    [self showTutorial];
  }
}
...
- (void)showTutorial {
  ...
  label.text = @"Tap on the apps you want to be in the list, and checkmarks "
               @"will indicate which apps are selected. The Blacklist / "
               @"Whitelist setting in the previous menu determines what "
               @"function this app list serves.\n\nTap anywhere to continue.";
